...
class XslInputData:
    def __init__(self, input_file):
        ...
        self.m = int(self.sheet1['B3'].value)  # vado a leggere il numero di provette che possono essere processate contemporaneamente
    ...
    def read_jobs(self):
        ...
        for i in range(self.jobs_num):
            list_task_job = []
            for j in range(self.tasks_num):
                list_task_job.append(Task(j, matrix_TJ[i][j] * duration_t[j]))  # per ciascun jobs, vado a leggere e tenere in
                # memoria la durata dei task (la durata sarà 0 per i tasks non presenti in quello specifico jobs
            jobs.append(Job(i, self.sheet_job.cell(i + 2, 2).value, self.sheet_job.cell(i + 2, 3).value, list_task_job))  # inizializzo il
